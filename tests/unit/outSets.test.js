...
describe('재고 「나감」 — 도급·판금은 세트 기준, 사급은 체크 기준', () => {
  ...
  it('도급: 9대가 시작했고 5대만 다 채웠으면 「9 SET · 부족 4」', () => {
    ...
    expect(outSetsOf(t, 'A', nine)).toEqual({ sets: 9, full: 5, short: 4 });
  ...
  it('도급: 「제외」로 꺼 둔 호기는 부족에서 뺀다 — 세트 수는 그대로 9 (대표님 「나감 9로 맞춰주고」)', () => {
    ...
    expect(outSetsOf(t, 'A', nine)).toEqual({ sets: 9, full: 5, short: 0 });
  ...
  it('사급: 이 줄에 실제로 넣은 호기만 센다 (대표님 「체크된 호기에 한해서」)', () => {
    const t = outTally();
    tallyOut(t, 'A', 'p1', 2, 2);
    tallyOut(t, 'A', 'p2', 2, 2);
    tallyOut(t, 'A', 'p3', 1, 2); // 덜 넣음
    tallyOut(t, 'A', 'p4', 0, 2); // 안 넣음 — 안 센다
    expect(outSetsOf(t, 'A')).toEqual({ sets: 3, full: 2, short: 1 });
    expect(outSetsLabel(outSetsOf(t, 'A'))).toBe('3 SET · 부족 1');
  ...
  it('같은 품목 다른 줄에서 덜 채운 호기는 다 채운 것으로 안 센다', () => {
    ...
    expect(outSetsOf(t, 'A', new Set(['p1']))).toEqual({ sets: 1, full: 0, short: 1 });
  });
});
