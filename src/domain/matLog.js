...
// 호기 타입이 바뀌어 지금 타입에 없어진 줄을 비울 때 — «보라 줄»에서만 고를 수 있다.
// 「미입고」로 남기면 왜 빠졌는지 나중에 알 수 없어 자재 이력에서 못 거른다
// (2026-09-17 대표님 「초과된 부분은 빼기 사유에 타입변경」).
// 실물은 안 쓰게 된 물건이라 재고 통으로 돌아간다 — 「미입고」와 같은 취급.
export const TYPE_CHANGE = '타입 변경';
// BOM 의 필요 수량이 나중에 줄어, 이미 체크해 둔 개수가 넘칠 때 그 초과분을 빼는 까닭.
// 「미입고」로 적으면 «안 들어온 것»이 되어 부족 집계에 잡히고, 「제외」면 통으로 안 돌아간다.
// 실물은 안 쓰게 된 물건이라 재고 통으로 돌아간다 — 「미입고」·「타입 변경」과 같은 취급
// (2026-09-18 대표님 「A B 전부 ㄱㄱ」).
export const QTY_DOWN = '수량 줄어듦';
export const OUT_WHYS_STRAY = [TYPE_CHANGE, '가져감'];
export const OUT_WHYS_OVER = [QTY_DOWN, '가져감'];
